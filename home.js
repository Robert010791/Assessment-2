...
const canWeDeliver = (zipCode) => {
    for (i = 0; i < deliveryAreaZipCodes.length; i++){
        if (deliveryAreaZipCodes[i] === zipCode) {
            return 'We can deliver to you friend!!'
        } else {
            return "Sorry but you ain't getting no food"
        }
    }
}
...
const canWeDeliverTwo = (deliveryZip) =>{
    if (deliveryAreaZipCodes.includes(deliveryZip) === true){
    return 'Enjoy your food fam!'
    } else {
        return 'Sorry you live in the middle of nowhere'
    }
} 
...
let {title} = deals
let newDiscount = title.replace('15', '10')
